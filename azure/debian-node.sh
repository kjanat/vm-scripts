#!/usr/bin/env bash
# Set up a Debian-based Azure VM with Node.js, Bun, Deno, and related tools.
#
# From a local clone:
#   sudo ./azure/debian-node.sh
#
# Download + run (fetches install scripts automatically):
#   curl -fsSL https://raw.githubusercontent.com/kjanat/vm-scripts/master/azure/debian-node.sh -o debian-node.sh
...
if [[ -f "${_repo_dir}/install/_common.sh" ]]; then
	...
	for _f in _common.sh shfmt.sh just.sh bun.sh deno.sh dprint.sh volta.sh azcli.sh aliases.sh; do
...
# Provider CLI
# ---------------------------------------------------------------------------
# shellcheck source=install/azcli.sh
source "${INSTALL_DIR}/azcli.sh"
